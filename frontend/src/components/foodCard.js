    ...
    const handleSubmit = async (event, foodData) => {
        ...
        axios.get(getURL)
            .then(res => {
                ...
                setFoodsEaten(res.data.food_eaten);
                setFoodsEatenCalories(res.data.sum_calories.calories);
                setCalsLeftToday(res.data.daily_calories.daily_calories - res.data.sum_calories.calories)
                setFat(res.data.sum_fat.fat)
                setProtein(res.data.sum_protein.protein)
                setCarbs(res.data.sum_carbs.carbs)
            ...
            <div>
                <div className="flex justify-center mt-5">
                    <div className="w-full lg:w-8/12 border-2 rounded-md ">
                        <h2 className="mt-3 mb-3 justify-center ml-4 flex text-2xl lg:text-4xl font-bold">Foods eaten
                            by {userData.name} </h2>
                        <div className="grid grid-cols-1 place-items-center">
                            <div className="ml-4 mt-2 mb-3 text-md lg:text-2xl font-bold text-center">
                                <div className="grid grid-cols-2">
                                    <div
                                        className="border-2 mr-5 p-5 rounded-xl border-gray-100/60 shadow shadow-gray-100/50 ">
                                        <p>Calories Consumed Today: <span
                                            className="text-green-500">{userEatenCalories}</span>
                                        </p>
                                    ...
                                    <div
                                        className="border-2 ml-5 p-5  rounded-xl border-gray-100/60 shadow shadow-gray-100/50">
                                        <p>Calories Left Today: <span className="text-red-400">{calsLeftToday}</span>
                                        </p>
                                        <div className="grid grid-cols-3 text-center">
                                            <p>Fat </p>
                                            <p>Carbs</p>
                                ...
                                <button className={`w-full ${deleteButton}`}
                        ...
                        <div className="flex justify-center grid grid-cols-1 ml-8">
                            <DropDown
                                marginLeft="ml-2" width="w-11/12" height="h-2/6"
                                title="foods"
                                    ...
                                    <tr className="border px-4 py-2 text-center" key={index}>
                                        ...
                                        <td className="pr-7">
                                            <button
                                                ...
                                                onClick={(e) => handleSingleDelete(e, food.id, current_Id)}>
                                                Delete
                                            </button>
                                        </td>
                                    </tr>
